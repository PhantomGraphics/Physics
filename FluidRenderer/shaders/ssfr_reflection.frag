...
void main() {
    ...
    if (hasEnvMap != 0) {
        ...
        vec3 normal = normalize(cross(posR - viewPos, posU - viewPos));

        vec3 viewDir  = normalize(viewPos);
        vec3 reflView = reflect(viewDir, normal);
        vec3 reflWorld = mat3(invViewRot) * reflView;
        ...
        float fresnel = pow(1.0 - clamp(depth, 0.0, 1.0), 2.0);
        color = tint.rgb * mix(0.35, 1.0, f + fresnel * 0.3);
    }
